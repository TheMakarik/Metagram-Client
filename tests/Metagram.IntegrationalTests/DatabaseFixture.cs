...
public sealed class DatabaseFixture : IAsyncLifetime
{
    ...
    private static readonly Faker<User> UserFaker = new Faker<User>()
        ...
        .RuleFor(static user => user.Username, static (_, user) => "@" + user.FirstName)
    ...
    private static readonly Faker<Message> MessageFaker = new Faker<Message>()
        .RuleFor(static message => message.BotChatId, static faker => faker.Random.Int(0, BotChatsCount - 1))
    ...
    public async Task InitializeAsync()
    {
        ...
        using (_connection = TestSqliteConnectionFactory.Instance.GetFactory())
        {
            _connection.Open();
   ...
   VALUES(@TelegramMessageId, @BotChatId, @MediaPath, @EditedAt, @SentAt)";
    ...
    private async Task GenerateAndExecuteQueryAsync<TEntity>(Faker<TEntity> faker, int count, string query, int retriesCount = 0) where TEntity : class
    {
        ...
        try
        {
            await _connection.ExecuteAsync(query, entities);
        }
